...
const updateCurrentMonth = () => {
  ...
  try {
    execSync('node fetch-current-month.js', { stdio: 'inherit' });
    console.log('✅ Current month updated successfully\n');
  } catch (error) {
...
const loadData = () => {
  const jsonPath = path.join(__dirname, 'story-point-all-updates-data.json');
  return fs.existsSync(jsonPath) ? JSON.parse(fs.readFileSync(jsonPath, 'utf8')) : null;
};

const loadTextReport = () => {
  const reportPath = path.join(__dirname, 'story-point-all-updates-report.txt');
...
app.get('/', (req, res) => {
  ...
  if (!data) {
    return res.send('<h1>No data found. Please run script-all-updates.js first.</h1>');
  }
  ...
  const calculateAverageEffort = (month) => {
    if (!month.distribution) return 0;
    let totalEffort = 0;
    let totalTasks = 0;
    Object.entries(month.distribution).forEach(([points, count]) => {
      if (points !== 'null') {
...
<head>
  ...
  <title>All Story Point Updates Dashboard</title>
  ...
  <div class="container">
    <div class="header">
      ...
      <div style="margin-top: 20px;">
        <label style="font-size: 1em; color: #666; margin-right: 10px;">Filter by Base Story Point:</label>
        <select onchange="window.location.href='/?base='+this.value" style="padding: 10px 20px; font-size: 1em; border-radius: 8px; border: 2px solid #667eea; background: white; cursor: pointer;">
    ...
    <div id="distribution" class="tab-content">
      <div class="data-table">
        <h2 class="chart-title">📊 Story Point Distribution by Month</h2>
        <p style="text-align: center; color: #666; margin-bottom: 20px;">Shows how many tasks have each story point value in each month</p>
          ...
          return `
            <div style="margin-bottom: 40px; padding: 25px; background: #f8f9fa; border-radius: 10px; border-left: 4px solid #667eea;">
              ...
              <table style="width: 100%; margin-bottom: 0;">
                <thead>
                  <tr>
                    <th style="text-align: left; width: 150px;">Story Points</th>
                ...
                <tbody>
                  ${sortedPoints.map(points => {
                    ...
                    const label = points === 'null' ? 'No Story Points' : `${points} points`;
      ...
      <div class="data-table">
        <h2 class="chart-title">📈 Story Point Distribution Trends</h2>
...
app.listen(PORT, () => {
  console.log(`\n🚀 All Updates Dashboard running at http://localhost:${PORT}`);
